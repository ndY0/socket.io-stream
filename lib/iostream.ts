...
export class IOStream extends Duplex {
  ...
  constructor(public options: DuplexOptions) {
    super(options);
  ...
  public _read = function(size: number) {
    var push;
  ...
  public _onread(size: number) {
    var write = this.writeBuffer.shift();
    if (write) return write();
  
    (this as any)._writable = true;
  }
  public _write(chunk: any, encoding: BufferEncoding, callback: Function) {
    var self = this;
    ...
    function write() {
      ...
      (self as any)._writable = false;
      self.socket._write(self.id, chunk, encoding, callback);
    }
  
    if ((this as any)._writable) {
      write();
    } else {
      this.writeBuffer.push(write);
    }
  }
  public _onwrite = function(chunk: any, encoding: BufferEncoding, callback: Function) {
    var self = this;
  ...
  public _onfinish() {
    ...
    // this.writable = false;
    (this as any)._writableState.ended = true;
  
    if (!this.readable || (this as any)._readableState.ended) {
      return this.destroy();
    }
    ...
    if (!this.allowHalfOpen) {
      ...
      if (this.readable && !(this as any)._readableState.endEmitted) {
        this.read(0);
      }
    }
  }
  public _onend() {
    this.readable = false;
    (this as any)._readableState.ended = true;
  
    if (!this.writable || (this as any)._writableState.finished) {
      return this.destroy();
    }
